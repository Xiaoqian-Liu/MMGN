...
#' @param max_iter maximum number of iterations (default: 5e2)
...
#' @export
#' 
MMGN_probit_auto <- function(Y, omega, sigma, rSeq=1:5, rate=0.8, seed=1234, maxiters=5e2, tol=1e-4, stopping='objective'){
  ...
  for (i in 1:numR) {
    ...
    res <- MMGN_probit(Y, omega = training, R=r, sigma = sigma, maxiters = maxiters, tol=tol, stopping=stopping)
    Mhat <- res$U%*%t(res$V)
  ...
  res <- MMGN_probit(Y, omega = omega, R=rhat, sigma = sigma, maxiters = maxiters, tol=tol, stopping=stopping)
  iter <- res$iters
  
  return(list(U = res$U, V = res$V, rhat = rhat, loglik = loglik, iters=iter, obj = res$obj[1:iter],
              relchange = res$relchange[1:iter], dec = res$dec[1:iter], 
              alphas = res$alphas[1:iter], nBacktracks = res$nBacktracks[1:iter]))
}
