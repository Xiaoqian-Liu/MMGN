function [Mhat, rhat, err, outs] = Manopt_logist_auto(Y, ind_omega, sigma, M, opts)
% This function implements Manifold optimization (Manopt) for 1-bit matrix completion
% under the logistic noise model with a data driven approach for selecting the rank
...
%% main loop to select r
    for k = 1:numR
        ...
        [Mhat1, relerr1] = Manopt_logist(Y, ind_train, sigma, r, U0, V0, M, 'maxiters',maxiters, 'tol',tol, 'verbosity', verbosity);
    ...
    [Mhat, err] = Manopt_logist(Y, ind_omega, sigma, rhat, U00, V00, M, 'maxiters',maxiters, 'tol',tol, 'verbosity', verbosity);
