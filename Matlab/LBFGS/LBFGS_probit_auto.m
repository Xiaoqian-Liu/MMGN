function [U, V, rhat, err, out, outs] = LBFGS_probit_auto(Y, ind_omega, sigma, M, opts)
% This function implements LBFGS for 1-bit matrix completion under the
% probit noise model with a data-driven approach for selecting the rank constraint
% parameter r.
...
%% main loop to select r
    for k = 1:numR
        ...
        [U, V, ~] = LBFGS_probit(Y, ind_train, sigma, r, U0, V0, 'maxiters',maxiters, 'tol',tol,...
    ...
    [U, V, out] = LBFGS_probit(Y, ind_omega, sigma, rhat, U00, V00,'maxiters',maxiters, 'tol',tol,...
                                                   'm', mm, 'DisplayIters', DisplayIters);
